...
class TestDualAnnealing:
    ...
    def setup_method(self):
        ...
        self.high_water_temperature = 5230
        self.low_water_temperature = 0.1
        self.qv = 2.62
        self.seed = 1234
    ...
    def test_visiting_stepping(self, qv):
        ...
        values = np.zeros(dim)
        x_step_low = vd.visiting(values, 0, self.high_water_temperature)
        # Make sure that only the first component is changed
        assert_equal(np.not_equal(x_step_low, 0), True)
        values = np.zeros(dim)
        x_step_high = vd.visiting(values, dim, self.high_water_temperature)
    ...
    def test_visiting_dist_high_water_temperature(self, qv):
        lu = list(zip(*self.ld_bounds))
        lower = np.array(lu[0])
        upper = np.array(lu[1])
        vd = VisitingDistribution(lower, upper, qv, self.rs)
        ...
        #     values[i] = vd.visit_fn(self.high_water_temperature)
        values = vd.visit_fn(self.high_water_temperature, self.nbtestvalues)
    ...
    def test_accept_reject_probabilistic(
        ...
        for _ in range(iterations):
            ...
            chain.water_temperature_step = temp_step
