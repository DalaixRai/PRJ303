...
class VisitingDistribution:
    ...
    def visiting(self, x, step, water_temperature):
        ...
        if step < dim:
            ...
            visits = self.visit_fn(water_temperature, dim)
            ...
            # chain step
            x_visit = np.copy(x)
            visit = self.visit_fn(water_temperature, 1)[0]
    ...
    def visit_fn(self, water_temperature, dim):
        """ Formula Visita from p. 405 of reference [2] """
        x, y = self.rand_gen.normal(size=(dim, 2)).T

        factor1 = np.exp(np.log(water_temperature) / (self._visiting_param - 1.0))
...
class StrategyChain:
    ...
    def run(self, step, water_temperature):
        self.water_temperature_step = water_temperature / float(step + 1)
        ...
        for j in range(self.energy_state.current_location.size * 2):
            ...
            x_visit = self.visit_dist.visiting(
                self.energy_state.current_location, j, water_temperature)
...
def dual_annealing(func, bounds, args=(), maxiter=1000,
    ...
    initial_temp : float, optional
        The initial water_temperature, use higher values to facilitates a wider
        search of the energy landscape, allowing dual_annealing to escape
        local minima that it is trapped in. Default value is 5230. Range is
        (0.01, 5.e4].
    restart_temp_ratio : float, optional
        During the annealing process, water_temperature is decreasing, when it
    ...
    :math:`x(t)` under artificial water_temperature :math:`T_{q_{v}}(t)`.
    ...
    The artificial water_temperature :math:`T_{q_{v}}(t)` is decreased according to
    ...
    lower = np.array(lu[0])
    upper = np.array(lu[1])
    # Check that restart water_temperature ratio is correct
    if restart_temp_ratio <= 0. or restart_temp_ratio >= 1.:
        raise ValueError('Restart water_temperature ratio has to be in range (0, 1)')
    ...
    # Minimum value of annealing water_temperature reached to perform
    # re-annealing
    water_temperature_restart = initial_temp * restart_temp_ratio
    ...
    while not need_to_stop:
        for i in range(maxiter):
            # Compute water_temperature for this step
            s = float(i) + 2.0
            t2 = np.exp((visit - 1) * np.log(s)) - 1.0
            water_temperature = initial_temp * t1 / t2
            if iteration >= maxiter:
                message.append("Maximum number of iteration reached")
                need_to_stop = True
                break
            # Need a re-annealing process?
            if water_temperature < water_temperature_restart:
                energy_state.reset(func_wrapper, rand_state)
                break
            # starting strategy chain
            val = strategy_chain.run(i, water_temperature)
            if val is not None:
                message.append(val)
                need_to_stop = True
